...
void RosNode::create_robot_instance() {
  ...
  taser_cpp::RobotConfig config{
      ...
      taser_cpp::navigation::Dimensions{
  ...
  robot_ = std::make_unique<taser_cpp::Robot>(config);
...
taser_cpp::Transform RosNode::get_transform(std::string target_frame,
                                        std::string source_frame) const {
  try {
    auto tf =
        buffer_.lookupTransform(target_frame, source_frame, tf2::TimePointZero);
    return {
        taser_cpp::Vector3(tf.transform.translation.x, tf.transform.translation.y,
                       tf.transform.translation.z),
        taser_cpp::Quaternion(tf.transform.rotation.w, tf.transform.rotation.x,
                          tf.transform.rotation.y, tf.transform.rotation.z)};
  } catch (tf2::TransformException &ex) {
    return {taser_cpp::Vector3(0, 0, 0), taser_cpp::Quaternion(1, 0, 0, 0)};
  }
}

taser_cpp::Transforms RosNode::get_arm_transforms(std::string arm_name) const {
  taser_cpp::Transforms tfs;
...
void RosNode::set_robot_pose_in_sim(const taser_cpp::Pose2D &pose) const {
...
void RosNode::spawn_obstacles() {
  ...
  std::vector<taser_cpp::Obstacle> obstacles;
  ...
  for (int i = 0; i < obstacle_count; i++) {
    ...
    auto obstacle = taser_cpp::navigation::get_box_corners(position[0], position[1],
                                                 size[0], size[1]);
...
void RosNode::move_arms(const std::shared_ptr<MoveArmsGoalHandle> goal_handle) {
  ...
  taser_cpp::Position3D l_p;
  taser_cpp::Position3D r_p;
  ...
  if (goal->target_id != -1) {
    ...
    if (target != targets_.end()) {
      auto target_pose_map =
          taser_cpp::Pose3D{{target->pose.position.x, target->pose.position.y,
      ...
      l_p = taser_cpp::Position3D{target_pose_base.position.x,
                              target_pose_base.position.y + 0.5,
                              target_pose_base.position.z};
      r_p = taser_cpp::Position3D{target_pose_base.position.x,
                              target_pose_base.position.y - 0.5,
                              target_pose_base.position.z};
    } else {
    ...
    l_p =
        taser_cpp::Position3D{goal->left_arm.x, goal->left_arm.y, goal->left_arm.z};
    r_p = taser_cpp::Position3D{goal->right_arm.x, goal->right_arm.y,
                            goal->right_arm.z};
  }
...
double RosNode::move_arm_step(std::string name, taser_cpp::Position3D p) const {
