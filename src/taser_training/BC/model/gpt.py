...
from taser_training.BC.model.config import GPTCfg
...
class SelfAttention(nn.Module):
    def __init__(self, config: GPTCfg):
        super().__init__()
...
class GPT(nn.Module):
    def __init__(self, config=GPTCfg()):
        ...
        self.transformer = nn.ModuleDict(
            dict(
                token_embed=nn.Linear(config.obs_dim, config.n_embd),
                pos_embed=nn.Embedding(config.seq_len, config.n_embd),
        ...
        self.action_head = nn.Linear(
            config.n_embd, config.action_dim * config.action_chunk_size
        )

        if config.predict_eef_pos:
            self.eef_pos_head = nn.Linear(config.n_embd, 6 * config.action_chunk_size)
...
if __name__ == "__main__":
    ...
    x = torch.randn(10, model.config.seq_len, model.config.obs_dim)
    out = model(x)
