...
def lin_vel_error_too_large(env: ManagerBasedEnv):
    return torch.abs(env.target_vel_b[:, 0] - root_planar_vel_b(env)[:, 0]) > env._max_lin_vel * 1.2


def ang_vel_error_too_large(env: ManagerBasedEnv):
    return torch.abs(env.target_vel_b[:, 1] - root_planar_vel_b(env)[:, 1]) > env._max_ang_vel * 1.2
