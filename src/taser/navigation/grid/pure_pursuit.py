...
if __name__ == "__main__":
    # Small demo: follow an S-shaped polyline

    path = [(0.0, 0.0), (1.0, 0.0), (2.0, 0.8), (3.0, 0.8), (4.0, 0.0), (5.0, 0.0)]
    ...
    for _ in range(1200):  # up to 60 s
        cmd, reached, info = pp.step(pose, v_current=v_curr)
    ...
    # Plot path and executed trajectory
    xs, ys = zip(*path)
    plt.figure()
    plt.plot(xs, ys, linestyle="--", marker="o", label="Path")
    plt.plot(traj_x, traj_y, label="Trajectory")
    plt.axis("equal")
    plt.legend()
    plt.title("Pure Pursuit Demo")
    plt.xlabel("x [m]")
    plt.ylabel("y [m]")
    plt.show()
