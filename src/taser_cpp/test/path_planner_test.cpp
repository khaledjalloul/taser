...
protected:
  ...
  taser_cpp::navigation::Dimensions dim_{-2, 5, -2, 5};
  taser_cpp::navigation::PathPlanner rrt_{200, 0.1, 0.5, 0.0, dim_};
};

TEST_F(PathPlannerTest, check_collision) {
  std::vector<taser_cpp::Obstacle> obstacles{{{1, 1}, {1, 2}, {2, 2}, {2, 1}}};
  rrt_.set_obstacles(obstacles);

  taser_cpp::navigation::DubinsSegment dubins_seg;
...
TEST_F(PathPlannerTest, generate_path_no_obstacles) {
  auto start = taser_cpp::Pose2D{0, 0};
  auto goal = taser_cpp::Pose2D{3, 3};
