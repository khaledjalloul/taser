...
#include "taser_cpp/manipulation/controller.hpp"
#include "taser_cpp/manipulation/kinematics.hpp"
#include "taser_cpp/navigation/controller.hpp"
#include "taser_cpp/navigation/path_planner.hpp"
...
struct RobotConfig {
  ...
  navigation::Dimensions base_rrt_dim; // Dimensions of the environment for the
                                       // RRT* path planner to sample from
};
...
public:
  ...
  std::unique_ptr<locomotion::Kinematics> base;

  std::map<std::string, manipulation::Kinematics> arms;
  std::unique_ptr<manipulation::Controller> arm_controller;

  std::unique_ptr<navigation::Controller> base_controller;
  std::unique_ptr<navigation::PathPlanner> rrt;
