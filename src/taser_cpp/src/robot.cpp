...
Robot::Robot(const RobotConfig &config) : dt(config.dt) {
  arms = {{"left", manipulation::Kinematics("left")},
          {"right", manipulation::Kinematics("right")}};
  arm_controller =
      std::make_unique<manipulation::Controller>(config.arm_controller_kp);

  base = std::make_unique<locomotion::Kinematics>(config.base_L,
                                                  config.base_wheel_radius, dt);
  base_controller =
      std::make_unique<navigation::Controller>(dt, config.base_mpc_horizon);
  rrt = std::make_unique<navigation::PathPlanner>(
      config.base_rrt_num_samples, dt, config.base_L, config.base_velocity,
      config.base_rrt_dim);
}
...
std::tuple<double, double, double> Robot::move_base_step() {
  ...
  auto err = navigation::get_euclidean_distance(base->pose, path_.back());
