...
def plot_dubins_segment(
    ...
    arc = segment.arc
    start_theta = (
        np.arctan2(arc.start.y - arc.center.y, arc.start.x - arc.center.x) * 180 / np.pi
    )
    ...
    if quiver:
        plt.quiver(
            arc.start.x,
            arc.start.y,
            np.cos(arc.start.theta),
            np.sin(arc.start.theta),
            color="black",
            scale_units="xy",
            zorder=2,
            scale=1.3,
        ...
        plt.quiver(
            line.end.x,
            line.end.y,
            np.cos(line.end.theta),
            np.sin(line.end.theta),
            color="black",
            scale_units="xy",
            zorder=2,
            scale=1.3,
